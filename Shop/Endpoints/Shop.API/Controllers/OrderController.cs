...
public class OrderController : ApiController
{
    ...
    public async Task<ApiResult<OrderFilterResult>> GetUserOrdersByFilter(int pageId = 1, int take = 10, OrderStatus status = OrderStatus.Finalized)
    {
    ...
    public async Task<ApiResult> IncreaseOrderItemCount(IncreaseOrderItemQuantityCommand command)
    {
        var result = await _orderFacade.IncreaseItemCount(command);
        return CommandResult(result);
    }
    [HttpPut("orderItem/DecreaseCount")]
    public async Task<ApiResult> DecreaseOrderItemCount(DecreaseOrderItemQuantityCommand command)
    {
