...
public class UserAddress : BaseEntity
{
        ...
        PhoneNumber phoneNumber, string name, string family, string nationalCode)
    {
        ...
        var svc = new PostalCodeService(http);
        if (!svc.IsValidIranPostalCode(postalCode))
        {
            var errs = svc.GetValidationErrors(postalCode);
            throw new InvalidDomainDataException(string.Join(" | ", errs));
